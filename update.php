...
if(isset($_GET['ide']))
{
$sql=mysqli_query($conn,"select * from users where id='{$_GET['ide']}'");
...
if($newpic)
{
	if(move_uploaded_file($_FILES['file']['tmp_name'],"upload/".$_FILES['file']['name']))
	{
		...
		$query=mysqli_query($conn,"UPDATE users SET 
										...
										`country` = '$country', 
										`dob` = '$dob' 
										WHERE `users`.`id` = '{$_GET['ide']}'")or die(mysqli_error($conn));
...
<table align="center" bgcolor="#999999" border="2" bordercolor="#0000FF">
		...
		<tr>
				...
				<td>
					...
					writing<input type="checkbox" name="hobb[]" value="singing"
					<?php in_array('writing',$hobb)? print "checked":""; ?> /> 
					reading<input type="checkbox" name="hobb[]" value="dancing"
					<?php in_array('reading',$hobb)? print "checked":""; ?> ?>    				
				</td>
		</tr>
		<tr>
				<td>Country</td>
